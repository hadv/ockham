...
impl StateManager {
    ...
    pub fn commit_account(&self, address: Address, info: crate::storage::AccountInfo) -> Result<(), StateError> {
        // 1. Save full account data to persistent storage (for VM execution)
        self.storage.save_account(&address, &info).map_err(|e| StateError::Smt(e.to_string()))?;
    ...
    pub fn commit_storage(&self, address: Address, index: U256, value: U256) -> Result<(), StateError> {
        self.storage.save_storage(&address, &index, &value).map_err(|e| StateError::Smt(e.to_string()))
    }
}
...
impl Database for StateManager {
    ...
    fn basic(&mut self, address: Address) -> Result<Option<RevmAccountInfo>, Self::Error> {
        // Fetch from storage
        if let Some(info) = self.storage.get_account(&address).map_err(|e| StateError::Smt(e.to_string()))? {
            let code = if let Some(c) = info.code {
                Some(Bytecode::new_raw(c))
            } else if info.code_hash != Hash::default() {
                // Fetch code by hash
                let code_bytes = self.storage.get_code(&info.code_hash).map_err(|e| StateError::Smt(e.to_string()))?;
    ...
    fn code_by_hash(&mut self, code_hash: B256) -> Result<Bytecode, Self::Error> {
        let code_bytes = self.storage.get_code(&Hash(code_hash.0)).map_err(|e| StateError::Smt(e.to_string()))?;
    ...
    fn storage(&mut self, address: Address, index: U256) -> Result<U256, Self::Error> {
        self.storage.get_storage(&address, &index).map_err(|e| StateError::Smt(e.to_string()))
    }
